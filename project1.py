...
def Date_Showing_Parsing():
    ...
    for perforList in itemElements:
        ...
        realmName = perforList.find("realmName")
        ...
        if len(title) > 0:
            return {"제목": title.text, "장르": realmName.text, "시작일": startDate.text, "마감일": endDate.text, "장소":place.text, "지역":area.text}
...
def Area_Showing_Parsing():
    ...
    for perforList in itemElements:
        ...
        if len(title) > 0:
            return {"제목": title.text, "장르": realmName.text, "시작일": startDate.text, "마감일": endDate.text, "시/도":sido.text, "군/구":gugun.text, "장소": place.text}
